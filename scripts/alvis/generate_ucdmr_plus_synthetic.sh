...
CHECKPOINT="${CHECKPOINT:-$OUT_ROOT/residual_flow_plus/checkpoints/latest_finite.pt}"
if [[ ! -f "$CHECKPOINT" ]]; then
  echo "latest_finite.pt not found. Pass CHECKPOINT=/path/to/finite_epoch.pt explicitly." >&2
  exit 2
fi
TEACHER_ARGS=()
...
python -m ucdmr_flow_residual_plus.cli.generate_synthetic \
  ...
  --flow-max-delta "${FLOW_MAX_DELTA:-0.25}" \
