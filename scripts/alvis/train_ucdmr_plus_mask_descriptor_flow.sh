...
if [[ -z "${SLURM_JOB_ID:-}" && "${ALLOW_LOGIN_NODE:-0}" != "1" ]]; then
  echo "Refusing to train mask descriptor flow outside a Slurm allocation. Use salloc/sbatch, or set ALLOW_LOGIN_NODE=1 for a small CPU check." >&2
  exit 2
fi
...
python -m ucdmr_flow_residual_plus.cli.train_mask_descriptor_flow \
