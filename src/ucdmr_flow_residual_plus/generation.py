...
def dry_run_generate_summary(args: Any) -> dict[str, object]:
    ...
    return {
        ...
        "checkpoint": str(args.checkpoint or output_root / "residual_renderer_plus" / "checkpoints" / "latest.pt"),
...
def generate(args: Any) -> None:
    ...
    checkpoint_path = Path(args.checkpoint) if args.checkpoint else output_root / "residual_renderer_plus" / "checkpoints" / "latest.pt"
    ...
    with torch.no_grad():
        for idx, normal in enumerate(normals):
            ...
            if not candidates:
                continue
            mask_row = candidates[idx % len(candidates)]
            condition = _condition(mask_row)
            style = np.asarray([rng.gauss(0.0, 1.0) for _ in range(style_dim)], dtype=np.float32)
            ...
            records.append(
                {
                    ...
                    "mask_source": args.mask_source,
                    "residual_source": "plus_residual_renderer",
                    ...
                    "renderer_checkpoint": str(checkpoint_path),
                }
            )
