...
def train(args: Any) -> None:
    try:
        ...
        raise SystemExit("PyTorch is required for plus residual renderer training.") from exc
    ...
    from ucdmr_flow_residual_plus.training.losses import residual_loss

    config = load_config(args.config)
    dataset_root = resolve_dataset_root(config, args.dataset_root)
    output_root = resolve_output_root(config, args.output_root)
    pseudo_manifest = Path(args.pseudo_manifest) if args.pseudo_manifest else output_root / "pseudo_normal" / "pseudo_manifest.csv"
    train_root = Path(args.train_output) if args.train_output else output_root / "residual_renderer_plus"
    ...
    if state.is_main:
        ...
        print({"stage": "plus_residual_renderer", "rows": len(rows), "device": str(state.device), "distributed": state.distributed}, flush=True)
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(loader):
            ...
            m_raw = batch["m_raw"].to(state.device, non_blocking=True)
            ...
            with _autocast_context(torch, enabled=args.amp and torch.cuda.is_available()):
                delta = model(context, condition, domain_idx, style)
                loss, parts = residual_loss(
                    ...
                    m_band=m_band,
                    m_gate=m_gate,
                    lambda_inside=args.lambda_inside,
        ...
        if state.is_main:
            ...
            if (epoch + 1) % args.save_every == 0 or epoch == args.epochs - 1:
                ...
                ckpt = {"model": target_model.state_dict(), "optimizer": optimizer.state_dict(), "epoch": epoch, "global_step": global_step, "args": _jsonable_args(args), "metrics": metrics}
                torch.save(ckpt, ckpt_root / f"epoch_{epoch:04d}.pt")
                torch.save(ckpt, ckpt_root / "latest.pt")
            write_json(report_root / "plus_residual_renderer_training.json", {"history": history, "args": _jsonable_args(args)})
        barrier(torch, state)
    cleanup(torch)
